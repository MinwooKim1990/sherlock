...
> 생각하세요.
>
> 심층 리서치는 마지막에 **grounded 에디터 패스(v3, 기본 켜짐)**를 돌립니다: 모든
> 숫자를 수집한 출처에 재정합하고, 섹션 간·시점 모순을 바로잡고, "공식 사이트를
> 보세요" 식의 빈 섹션을 삭제하고, 결론을 맨 앞에 둡니다. 메인 모델을 쓰므로 (위
> 권장대로) 유능한 모델일수록 좋고, `deep_research_v3=False`로 끄면 이전 합성으로
> 돌아갑니다.
...
세 가지 LLM 역할을 모두 당신이 연결합니다(하나의 함수일 수도, 셋일 수도 있습니다) — 여기에
선택적 **네 번째 역할, 시각화 담당 LLM-4**(v1.12, 기본 off; 인라인 `<<sherlock-viz: …>>` 마커를
샌드박스 HTML/SVG로 렌더링 — 변경 이력 참고)가 더해집니다:
...
### v1.12 — 세션 간 장기 메모리 + LLM-4 인라인 시각화
두 가지 핵심 기능, 각각 off 상태는 바이트 동일합니다.
- **장기 메모리(기본 ON):** 이제 Sherlock이 *대화를 넘어* 지속되는 사실을 기억합니다. LLM-2가
  소수의 고가치 사실 — 신원/건강, 명시적 "기억해줘" 지시, 안정적 선호/관계/장기 프로젝트 — 을
  예약된 sentinel 스코프로 **승격(promote)**합니다. 무엇을 영구히 보관할 만큼 지속적인지는 모델이
  아니라 **코드 레벨 분류(taxonomy) 게이트**가 결정합니다(모델은 제안만, 결정은 언제나 코드). LLM-1은
  항상 켜진 **USER PROFILE** 블록과 sentinel RAG 채널로 이를 회상합니다. 저장·삭제·전체 삭제의
  **완전한 자연어 관리**는 단발성 확인 토큰으로 코드 게이트되어, 파괴적 동작이 모델의 말만으로는
  절대 실행되지 않습니다; **확인된 삭제**는 행과 그 벡터를 함께 하드 삭제합니다. **이식성:** Markdown /
  JSON / SQL로 내보내기·가져오기하며, 전체 삭제 전 fail-closed Markdown 백업을 남깁니다. 기본 ON이며,
  `config.memory.long_term.enabled=False`로 끌 수 있습니다(off는 v1.12 이전과 바이트 동일). **incognito**로
  전환하면 기존 사실은 회상하되 새 쓰기는 멈춥니다.
- **LLM-4 인라인 시각화(옵트인):** **네 번째 비동기 역할**입니다. 다이어그램·차트가 실제로 도움이 될 때
  LLM-1이 인라인 `<<sherlock-viz: …>>` 마커를 남기고, LLM-4가 **생성 → 정적 린트 → 자가 검토 →
  런타임 수리** 파이프라인을 거쳐 **샌드박스 iframe** HTML/JS 아티팩트로 렌더링합니다. 덕분에 작은
  모델도 깨진 시각화를 절대 내보내지 않습니다(끝내 유효하게 만들 수 없는 잡은 폐기되고 그 자리는
  일반 텍스트로 강등됩니다). viz 모델을 지정하지 않으면 메인 프로바이더로 폴백합니다.
  `config.visualization.enabled`로 옵트인하며, off(기본)에서는 마커 프로토콜이 휴면 상태이고 마커가
  그대로 남습니다(바이트 동일). 새 LLM-4 역할과 `memory.*` / `viz.*` 이벤트는
  [`docs/EVENTS.md`](docs/EVENTS.md)를 참고하세요.
- **콘텐츠에 맞는 시각화 & 이미지 생성:** LLM-4가 내용에 맞는 형식을 고릅니다 — 실제
  데이터는 차트, 구조는 SVG 다이어그램, 서사·삽화성 내용은 **생성 이미지**(`image:` 마커).
  **omni 모델**(이미지 출력 가능한 챗 모델)은 적응형 요청 사다리로 그대로 동작하고, 전용
  이미지 모델(`visualization.image_model`)도 지원하며, 텍스트 전용 모델은 SVG로 자연스럽게
  대체합니다. 보안은 fail-closed(아티팩트별 CSP `img-src` 정확-URL 핀), 크기는 콘텐츠에
  맞춰 동적으로 조절됩니다(`resize` 프로토콜 — 불필요한 여백 없음).
- **플레이그라운드: 영속 히스토리 + 한 화면 설정.** 대화가 프로필별로 저장되고 **LLM이
  제목을 생성**하며, 사이드바에서 다시 열기·이름 변경·삭제가 가능합니다(시각화는 디스크에서
  재복원). 연결 줄 하나 + 역할별 프로바이더/모델 그리드("모두 동일" 마스터 + 역할별
  오버라이드), 세션 중 모델 교체까지 지원합니다.
...
"수집 → 교차검증 → 통일성 있는 결과" 설계를 이번 릴리스에서 제대로 배선했습니다 — 각 LLM에 제
역할을 부여합니다. 가드레일은 **환각 방지 + 사실의 통일성에만** 한정되며, 포맷 / 길이 / 구조 /
출처 선택은 전적으로 모델의 판단에 맡깁니다.
- **LLM-2 충실성 검증**(`deep_research_verify="faithfulness"`, 기본): v3 에디터 이후, 별도의
  교차-모델 패스가 보고서를 수집된 **원문(raw)**과 대조(하위 주제별, 사실이 아닌 원문 — 사실 대조는
  순환이므로)하여 오추출(보고서는 X, 원문은 Y) + 동일-모델 에디터가 놓친 모순을 교정합니다.
  **비파괴적** — 교정만 하고 절대 삭제하지 않습니다(약한 검증자가 삭제하면 보고서가 비어버림);
  원문과 글자 단위로 일치하는 구간만, 상한·0.3 축소 가드 적용.
- **전체 보고서 통일성 스윕**(같은 플래그): 마지막 LLM-2 패스가 **섹션 간** 서로 다르게 적힌
  사실(여기선 9월 4-5일 / 저기선 9월 4-6일, 투어명 두 가지, 예/아니오가 둘 다)을 가장 근거 있는
  하나의 값으로 통일합니다(사실의 통일성).
- **LLM-3 웹 재검증**(옵트인, `deep_research_verify="faithfulness+web"`): 원문으로 결론이 안 나는
  소수의 주장만 새 웹 검색으로 재확인 → 확인 / 교정 / `[unverified]`(절대 조용한 덮어쓰기 없음).
  `deep_research_web_recheck_max`(3)로 상한.
- **구조화된 개체별 추출**(`deep_research_structured_extraction`, 기본 ON): 각 사실이 `entity` +
  `attrs`를 가질 수 있어, 묶인 속성(날짜·점수)이 *자기* 개체에 붙어 있게 합니다 — 작은 모델의
  개체-귀속 뒤바뀜(IVE 도시↔날짜 버그)을 막습니다.
- **신선도(freshness)**(`deep_research_freshness`, 기본 ON): 각 출처의 보고 날짜를 포착
  (DDG/Tavily/Brave/Valyu + 페이지 패치)해 모델에 노출하여, 가장 최신 출처를 선호하고 "지난 일을
  예정처럼" 적은 경우를 표시할 수 있게 합니다. 날짜는 불투명 문자열 — 코드에서 파싱·삭제하지 않음.
- **리치 라운드에서도 이미지**(`deep_research_fetch_image`, 기본 ON): 라운드가 얕지 않아도
  대표 `og:image`를 라운드당 한 번 수집.
- **인용 링크 수정**: `(unverified)` / `(pairing unverified)` 플래그를 마크다운 링크의 `)` *뒤*에
  붙이도록 수정(이전엔 URL 안에 끼워 넣어 링크가 깨졌음). 일반/괄호 URL은 그대로, 접두사 안전.
- **원문 영속화**(`deep_research_persist_raw`, **기본 OFF**): 런의 원문 조각을 SQLite에 저장해
  사후 회수("또 뭘 찾았어?")하는 옵트인. 정확도가 아닌 저장 증가 항목이라 기본 비활성.
- 모든 플래그의 `off`/`"off"` 값은 이전 동작과 **바이트 동일**. 라이브 평가 결과: 진실성 상승
  (~45% → ~78%), 섹션 간 자기모순과 깨진 링크 제거.
